		...
		if req.Method == "GET" {
			...
			if token == "" {
				...
				if err == nil {
					...
					logHttpRequest(accLog, req, http.StatusForbidden)
					return
				} else {
					errLog.Println(err)
					logHttpRequest(accLog, req, http.StatusForbidden)
			...
			if err == nil {
				...
				logHttpRequest(accLog, req, http.StatusMethodNotAllowed)
				return
			} else {
				logHttpRequest(accLog, req, http.StatusInternalServerError)
				errLog.Println(err)
...
func BotRouter(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if len(message.Entities) != 0 && message.Entities[0].Type == "bot_command" {
			...
			logHttpRequest(appContext.AccessLogger, req, http.StatusOK)
